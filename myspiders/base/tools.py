...
cookie_url_jschina = 'http://www.jsbchina.cn/CN/zygg/index.html?flag=0'
...
async def fetch_bank_cookies(bank_name):
    ...
    if bank_name == '浦发银行':
        ...
        cookie = await fetch_cookies_jschina()

    return cookie


async def fetch_cookies_jschina():
    lua = lua_script % cookie_url_jschina
    ...
    async with aiohttp.ClientSession() as client:
        ...
        for cookie in cookies:
            ...
            if name.startswith('Hm_lvt_') or name.startswith('Hm_lpvt_') or name.startswith('FSSBB'):
                query = name + '=' + value + ';'
                query_set.add(query)
        cookie_need = ''
        for query in query_set:
            cookie_need += query
        print('获取到的江苏银行Cookie是：%s' % cookie_need)
        return cookie_need
...
async def fetch_by_splash(link):
    lua = lua_script % link
    url = splash_url + quote(lua)
    async with aiohttp.ClientSession() as client:
        jsondata = await fetch_cookies_by_splash(client, url)
